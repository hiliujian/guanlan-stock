  ...
  methods: {
    ...
    init() {
      ...
      let chartdom = uni
        ...
        .boundingClientRect(data => {
          // 兜底：H5/动态内容首帧可能测不到尺寸（data 为 null 或宽高为 0），
          // 不重试会导致图表永远空白。延时重试最多 8 次，拿到有效尺寸再画。
          if (!data || !data.width || !data.height) {
            this._initRetry = (this._initRetry || 0) + 1;
            if (this._initRetry <= 8) {
              setTimeout(() => this.init(), 120);
              return;
            }
            // 重试耗尽仍测不到：用父容器的偏移宽高兜底，避免永远静默空白
            data = { width: (data && data.width) || 320, height: (data && data.height) || 220 };
          }
          this._initRetry = 0;
          if (data.width > 0 && data.height > 0) {
            this.mixinDatacomLoading = false;
...
export default {
  ...
  methods: {
    ...
    ucinit(newVal, oldVal, owner, instance){
      ...
      // 兼容：uni-canvas 包装器取 children[0]；若 H5 编译成裸 <canvas>（无子节点），
      // 直接用 canvasdom 自身取 2d context，否则取不到 context 会静默不画（图表空白）。
      let realCanvas = (canvasdom && canvasdom.children && canvasdom.children[0])
        ? canvasdom.children[0]
        : (canvasdom && typeof canvasdom.getContext === 'function' ? canvasdom : null)
      if(canvasdom && realCanvas){
        cfu.option[cid].context = realCanvas.getContext("2d")
        if(cfu.instance[cid] && cfu.option[cid] && cfu.option[cid].update === true){
          this.updataUChart()
        }else{
          setTimeout(()=>{
