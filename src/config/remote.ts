...
// 系统级配置（菜单显隐 / 数据源顺序等）由本地默认值 + 远程覆盖合成：
//   - app_config 表按 key 存 jsonb，key 为 "menus" / "sources"；
...
const DATA_KINDS: DataKind[] = ["realtime", "kline", "trend", "flow", "search", "news"];
...
export async function fetchRemoteSettings(): Promise<Partial<AppSettings>> {
  ...
  for (const r of rows) {
    if (r.key === "menus" && r.value && typeof r.value === "object") {
      ...
      out.menus = menus;
    } else if (r.key === "sources" && r.value && typeof r.value === "object") {
      const sources = {} as SourceConfig;
      for (const kind of DATA_KINDS) {
        const arr = (r.value as Record<string, unknown>)[kind];
        sources[kind] = Array.isArray(arr)
          ? (arr.filter((x): x is SourceId => typeof x === "string") as SourceId[])
          : [];
      }
      out.sources = sources;
    }
