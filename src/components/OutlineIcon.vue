...
const ICONS: Record<string, { body: string; filled?: boolean }> = {
  ...
  // 持仓（仓位 / 资产）：中国古铜钱（外圆内方），语义区别于自选「星标」与分组「layers」，
  // 几何仅「圆 + 方孔」两个图元，小尺寸不糊；全局统一用于「持仓」
  portfolio: {
    body: '<circle cx="12" cy="12" r="9"/><rect x="9" y="9" width="6" height="6" rx="0.5"/>',
  },
  // 持仓（激活态）：实心铜钱剪影，evenodd 挖出中央方孔
  "portfolio-filled": {
    body: '<path fill-rule="evenodd" d="M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18zm-2.5 6h5a.5.5 0 0 1 .5.5v5a.5.5 0 0 1-.5.5h-5a.5.5 0 0 1-.5-.5v-5a.5.5 0 0 1 .5-.5z"/>',
    filled: true,
  },
