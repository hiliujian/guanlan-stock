...
const ICONS: Record<string, { body: string; filled?: boolean }> = {
  ...
  // 持仓（资产组合 / 仓位）：堆叠币层（三层资产），语义区别于自选「星标」与分组「layers」，
  // 几何为「椭圆层 + 弧形币身」重复三次，缩放不糊；全局统一用于「持仓」
  portfolio: {
    body: '<ellipse cx="12" cy="6" rx="7.5" ry="3"/><path d="M4.5 6v4c0 1.66 3.36 3 7.5 3s7.5-1.34 7.5-3V6"/><path d="M4.5 10.5V14c0 1.66 3.36 3 7.5 3s7.5-1.34 7.5-3v-3.5"/><path d="M4.5 15v3.5c0 1.66 3.36 3 7.5 3s7.5-1.34 7.5-3V15"/>',
  },
  // 持仓（激活态）：实心剪影——同一组 path 闭合后整体填充，自然合并为实心币堆
  "portfolio-filled": {
    body: '<ellipse cx="12" cy="6" rx="7.5" ry="3"/><path d="M4.5 6v4c0 1.66 3.36 3 7.5 3s7.5-1.34 7.5-3V6z"/><path d="M4.5 10.5V14c0 1.66 3.36 3 7.5 3s7.5-1.34 7.5-3v-3.5z"/><path d="M4.5 15v3.5c0 1.66 3.36 3 7.5 3s7.5-1.34 7.5-3V15z"/>',
    filled: true,
  },
  // 切换 / 互换提示（自选 ↔ 持仓 品牌区点击切换）
  swap: {
