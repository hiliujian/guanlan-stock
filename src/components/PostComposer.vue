<template>
  <view>
    <view class="composer glass anim-fade-up">
      ...
      <view class="cp-area-wrap" ref="wrapRef">
        ...
        <!-- # 股票联想浮层（下拉）：锚定到 # 输入位置正下方悬浮显示。
             teleport 到 body：脱离发帖卡片（UniversalCard）的 overflow 裁剪，层级走公共 --z-popover -->
        <teleport to="body">
    ...
    <!-- 底部工具栏：+ 形变菜单 / 字数计数 / 发布。
         ...
         非独立弹窗）：文档流内撑高 composer 卡片，不受 UniversalCard transform/overflow 裁剪；
    ...
    <view :class="['cp-foot', menuOpen ? 'open' : '', editKind ? 'holding' : '']">
      ...
      <view ref="visWrapRef" class="cp-vis-wrap" @click.stop>
        ...
        <!-- 可见范围菜单：teleport 到 body，脱离发帖卡片（UniversalCard）的 overflow 与
...
/** 展开后把面板滚入可视区：卡片在 UniversalCard 滚动区内，避免展开部分落在可视区外看着像「没反应」 */
...
/* 脱离宿主（teleport 到 body）：视口固定定位，坐标由 JS 按入口换算；
   层级引用公共 --z-popover，保证浮层完整显示、不被发帖卡片（UniversalCard）裁剪 */
.cp-suggest.escaped {
  position: fixed;
  right: auto;
